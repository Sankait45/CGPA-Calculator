const subjects = [
    {
        name: "Open-Source Technologies (OE)", type: "Theory", credits: 2,
        components: [{key: "cia", label: "CIA", max: 20}, {key: "see", label: "SEE", max: 30}],
        checkPass: (v) => (v.cia >= 8) && (v.see >= 12)
    },
    {
        name: "Mathematics for Computer Engg.", type: "Theory + TW", credits: 3,
        components: [{key: "cia", label: "CIA", max: 40}, {key: "see", label: "SEE", max: 60}, {key: "tw", label: "TW", max: 25}],
        checkPass: (v) => (v.cia >= 16) && (v.see >= 24) && (v.tw >= 10)
    },
    {
        name: "Discrete Structures and Graph Theory", type: "Theory", credits: 3,
        components: [{key: "cia", label: "CIA", max: 40}, {key: "see", label: "SEE", max: 60}],
        checkPass: (v) => (v.cia >= 16) && (v.see >= 24)
    },
    {
        name: "Analysis of Algorithm", type: "Theory", credits: 3,
        components: [{key: "cia", label: "CIA", max: 40}, {key: "see", label: "SEE", max: 60}],
        checkPass: (v) => (v.cia >= 16) && (v.see >= 24)
    },
    {
        name: "Computer organization & Architecture", type: "Theory", credits: 3,
        components: [{key: "cia", label: "CIA", max: 40}, {key: "see", label: "SEE", max: 60}],
        checkPass: (v) => (v.cia >= 16) && (v.see >= 24)
    },
    {
        name: "Analysis of Algorithm Lab", type: "Lab", credits: 1,
        components: [{key: "tw", label: "TW", max: 25}, {key: "pr", label: "OR/PR", max: 25}],
        checkPass: (v) => (v.tw >= 10) && (v.pr >= 10)
    },
    {
        name: "Computer Organization and Architecture Lab", type: "Lab", credits: 1,
        components: [{key: "tw", label: "TW", max: 25}, {key: "pr", label: "OR/PR", max: 25}],
        checkPass: (v) => (v.tw >= 10) && (v.pr >= 10)
    },
    {
        name: "Full Stack Java Programming", type: "Lab", credits: 2,
        components: [{key: "tw", label: "TW", max: 50}, {key: "pr", label: "OR/PR", max: 25}],
        checkPass: (v) => (v.tw >= 20) && (v.pr >= 10)
    },
    {
        name: "Entrepreneurship Development", type: "Lab", credits: 2,
        components: [{key: "tw", label: "TW", max: 50}],
        checkPass: (v) => (v.tw >= 20)
    },
    {
        name: "Environmental Science for Engineers", type: "Lab", credits: 2,
        components: [{key: "tw", label: "TW", max: 50}],
        checkPass: (v) => (v.tw >= 20)
    }
];
...
function getGrade(percentage) {
    ...
    if (percentage >= 40) return { grade: "D", point: 4 }; // Note: Sem 3 uses 'D' instead of 'P'
...
if (saveBtn) {
    saveBtn.addEventListener("click", () => {
        if (window.currentSemData) {
            localStorage.setItem("sem3_data", JSON.stringify(window.currentSemData));
